...
window.findNQueensSolution = function(n) {
  ...
  var recurseChessBoard = function(options, queen){
    ...
    if(queen > n){
      if (!isFound) {
        solution = currentSolution.slice();
        isFound = true;
      }
      return
    }
  ...
  if (solution) {
    for (var i = 0; i < solution.length; i++) {
      board.togglePiece(solution[i][0], solution[i][1]);
    }
  }
...
window.countNQueensSolutions = function(n) {
  ...
  var recurseChessBoard = function(options, queen){
    ...
    if(queen > n){
      ...
      // console.log(currentSolution.slice());
      // solutionsArray.push(currentSolution.slice());
      var sortedSolution = currentSolution.slice().sort();
      var sortedSolutionString = JSON.stringify(sortedSolution.slice());
      var solutionsArrayString = JSON.stringify(solutionsArray.slice());
      if (!solutionsArrayString.includes(sortedSolutionString)) {
        solutionsArray.push(sortedSolution.slice());
      }
      // solutionsArray.push(currentSolution.slice());
      return;
    }
